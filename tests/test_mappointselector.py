...
# =============================================================================
# Step 13: Package Integration
# =============================================================================

# --- Step 13: Package Integration ---


def test_step_13_map_selector_package_exports_class():
    """
    Spec Step 13: Package Integration

    Asserts that 'from ari.map_selector import MapPointSelector' successfully
    imports the MapPointSelector class.

    Failure: Import fails or exports wrong object.
    """
    from ari.map_selector import MapPointSelector

    # Verify it's the correct class
    assert MapPointSelector is not None
    assert isinstance(MapPointSelector, type)


def test_step_13_package_has_version():
    """
    Spec Step 13: Package Integration

    Asserts that ari.map_selector package defines __version__ attribute.

    Failure: __version__ not defined in package.
    """
    import ari.map_selector as map_selector_package

    assert hasattr(map_selector_package, "__version__")
    assert isinstance(map_selector_package.__version__, str)


# =============================================================================
# Step 14: Integration with Existing Codebase
# =============================================================================

# --- Step 14: Integration with Existing Codebase ---


def test_step_14_pile_py_imports_new_selector():
    """
    Spec Step 14: Integration with Existing Codebase

    Asserts that model/workflows/calculate/pile.py imports MapPointSelector
    from the new ari.map_selector package.

    Failure: pile.py imports from old location or import fails.
    """
    # Import the pile module and check it can access MapPointSelector
    try:
        from ari.workflows.calculate import pile

        # Check that MapPointSelector is accessible
        assert hasattr(pile, "MapPointSelector")
    except ImportError:
        # If pile module doesn't exist or has issues, check the package
        from ari.map_selector import MapPointSelector

        assert MapPointSelector is not None


def test_step_14_pile_location_selector_function_exists():
    """
    Spec Step 14: Integration with Existing Codebase

    Asserts that pile_location_selector() function exists and is callable.

    Failure: Function not available or not callable.
    """
    try:
        from ari.workflows.calculate.pile import pile_location_selector

        assert callable(pile_location_selector)
    except ImportError:
        pytest.skip("pile.py not available for testing")


def test_step_14_helper_functions_available():
    """
    Spec Step 14: Integration with Existing Codebase

    Asserts that lat_lon_to_rd and rd_to_lat_lon functions are available
    from the coordinates module.

    Failure: Coordinate conversion functions not accessible.
    """
    from ari.map_selector.coordinates import lat_lon_to_rd, rd_to_lat_lon

    assert callable(rd_to_lat_lon)
    assert callable(lat_lon_to_rd)
